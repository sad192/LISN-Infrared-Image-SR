...
# 返回训练集或测试集目录下的所有图片相对路径加文件名 列表（list）
...
def calculate_weights_indices(in_length, out_length, scale, kernel, kernel_width, antialiasing):
    if (scale < 1) and antialiasing:
        ...
        kernel_width = kernel_width / scale # 获取更大的内核

    # Output-space coordinates
    x = torch.linspace(1, out_length, out_length) #返回一个1维张量，包含在区间start和end上均匀间隔的step个点。
    ...
    left = torch.floor(u - kernel_width / 2) #kernel_width ？
    ...
    indices = left.view(out_length, 1).expand(out_length, P) + torch.linspace(0, P - 1, P).view(
        1, P).expand(out_length, P)  # 获取索引矩阵
    ...
    if not math.isclose(weights_zero_tmp[0], 0, rel_tol=1e-6):
        indices = indices.narrow(1, 1, P - 2)  # 按列切片 返回Tensor的切片
...
if __name__ == '__main__':
    # 对测试集进行双三次下采样 1/4
    for filename in glob.glob(r'testImgs/*.png'):
        # print('ss',filename[9:-4])
        # break
        img = imread_uint(filename, 1)
        img = uint2single(img)
        # print('读取图片：', img)
        # img_bicubic = imresize_np(img, 1 / 4)
        # print('下采样后的图片：', img_bicubic.shape)
        img = modcrop(img, 4)
        # print('裁剪后', img.shape)
        # H, W, C = img_L.shape
        ...
        # print('裁剪后', img_L.shape)
        # break
        # mode = np.random.randint(0, 8)
        # print('增强策略: ', mode)
        # img_L = augment_img(img_L, mode=mode)
        # print('增强后', img_L.shape)
        # # imsave(single2uint(img_L), filename[9:-4]+'.bmp')
        # break
        # 双三次采样
        img_bicubic = imresize_np(img, 1 / 4)
        imsave(single2uint(img_bicubic), 'bicubicTest_result-A_x4/' + filename[9:-4] + '.png')
#
